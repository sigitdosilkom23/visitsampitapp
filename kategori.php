        ...
        <div class="container">
            <div class="w-100">
                ...
                if(isset($_GET['search'])){
                    ...
                    $count = $conn->query("SELECT * FROM `packages` where type = $query order by id DESC");
                } else{

                    $count = $conn->query("SELECT * FROM `packages` where type = $query order by id DESC");

                } ?>
                <h2 class="text-center">Kategori Wisata</h2>
                <div class="d-flex w-100 justify-content-center">
                    <hr class="border-warning" style="border:3px solid" width="15%">
                </div>
                <div class="row">
                    <h4 class="text-center"> Anda mencari wisata dengan Kategori <b>"<?php 
									if ($query==1) {
										echo 'Wisata Budaya';
									} elseif ($query==2) {
										echo 'Wisata Religi';
			...
			if(isset($_GET['search'])){
				...
				$packages = $conn->query("SELECT * FROM `packages` where type = $query order by id DESC");
			} else{
