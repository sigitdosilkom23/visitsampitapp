...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?= isset($_GET['id']) ? "Update ": "Create New " ?>Question Response</h3>
	</div>
	<div class="card-body">
		<form action="" id="response-form">
			...
			<?php else: ?>
				<div class="form-group">
					<label for="question_field" class="control-label">Question</label>
                    <input type="text" id="question_field" name="question[]" value="<?= isset($question) ? $question : '' ?>" class="form-control form-control-sm" required>
				</div>
			<?php endif; ?>
			<div class="form-group">
			<div class="continer-fluid" id="question-holder">
				...
				?>	
					<span class="badge badge-primary q-item m-2 pl-2">
						<span style='font-size:12px'><?= $_GET['q'] ?><input type='hidden' name='question[]' value='<?= $_GET['q'] ?>'></span>
			...
			<div class="form-group">
				...
				<textarea name="response_message" id="response_message" cols="30" rows="3" class="form-control" style="resize: none" required><?= isset($response_message) ? $response_message : ''; ?></textarea>
			</div>
		</form>
	</div>
