...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h3 class="card-title">Daftar Masukan/Feedback</h3>
            ...
            <table class="table table-stripped text-dark">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <p class="m-0"><b>User:</b> <?= ucwords($row['name']) ?></p>
                            <p class="m-0"><b>Wisata:</b> <?= ucwords($row['title']) ?></p>
                        </td>
                        <td>
                            <p class="truncate-1 m-0"><?= $row['rate'] ?></p>
                        </td>
                        <td>
                            <p class="truncate-1 m-0" title="<?= $row['review'] ?>"><?= $row['review'] ?></p>
                        </td>
                        <td>
                            <?php if ($row['status'] != 0) : ?>
                            <span>Published</span>
                        ...
                        <td align="center">
                            ...
                            <div class="dropdown-menu" role="menu">
                                <?php if ($row['status'] != 1) : ?>
                                <a class="dropdown-item accept_data" href="javascript:void(0)"
                                    data-id="<?= $row['id'] ?>"><span class="fa fa-check text-primary"></span>
                                    Published</a>
                                <?php endif ?>
                                <a class="dropdown-item delete_data" href="javascript:void(0)"
                                    data-id="<?= $row['id'] ?>"><span class="fa fa-trash text-danger"></span> Delete</a>
                            </div>
                        </td>
                    </tr>
                    <?php endwhile; ?>
...
<script>
    $(document).ready(function () {
        $('.delete_data').click(function () {
            _conf("Apakah kamu yakin menghapus masuksan secara permanen?", "delete_review", [$(this)
                .attr('data-id')
            ])
        })
        ...
        $('.accept_data').click(function () {
            _conf("Apakah anda yakin ingin mempublish review ini?", "accept_review", [$(this).attr(
                'data-id')])
        })
    })
    ...
    function accept_review($id) {
        start_loader();
        $.ajax({
            url: _base_url_ + "classes/Master.php?f=accept_review",
            method: "POST",
            data: {
                id: $id
            },
